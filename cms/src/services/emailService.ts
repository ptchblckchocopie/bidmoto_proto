...
export async function sendVoidRequestEmail(params: {
  ...
  return queueEmail({ to, subject, html, metadata: { type: 'void_request', productId, voidRequestId } });
}
...
export async function sendVoidResponseEmail(params: {
  ...
  return queueEmail({ to, subject, html, metadata: { type: 'void_response', productId, voidRequestId } });
}
...
export async function sendAuctionRestartedEmail(params: {
  ...
  return queueEmail({
    ...
    `,
    metadata: { type: 'auction_restarted', productId },
  });
}
...
export async function sendSecondBidderOfferEmail(params: {
  ...
  return queueEmail({
    ...
    `,
    metadata: { type: 'second_bidder_offer', productId, voidRequestId },
  });
}
...
export function renderTemplate(template: string, variables: EmailTemplateVariables): string {
  let result = template;
  const allVariables = { platformName: 'BidMo.to', platformUrl: process.env.FRONTEND_URL || 'http://localhost:5173', ...variables };
...
export async function getEmailTemplate(
  ...
  try {
    const templates = await payload.find({
      ...
      where: { type: { equals: type }, active: { equals: true } },
      limit: 1,
    });
    ...
    return { subject: template.subject, htmlContent: template.htmlContent, textContent: template.textContent };
  } catch (error) {
...
export async function sendTemplateEmail(
  ...
  return queueEmail({ to, subject, html, text, metadata });
}

export async function sendBidWonBuyerEmail(
  payload: Payload,
  params: { buyerEmail: string; buyerName: string; sellerName: string; productTitle: string; productId: number; bidAmount: number; currency: string }
): Promise<{ success: boolean; emailId?: string; error?: string }> {
  ...
  return sendTemplateEmail(payload, {
    to: buyerEmail, templateType: 'bid_won_buyer',
    variables: { buyerName, buyerEmail, sellerName, productTitle, productId, bidAmount, currencySymbol },
    fallback: {
      subject: `Congratulations! You won the bid for "${productTitle}"`,
      html: `<h2>Congratulations ${buyerName}!</h2><p>Your bid has been accepted for <strong>${productTitle}</strong>.</p><p><strong>Winning Bid:</strong> ${currencySymbol}${bidAmount.toLocaleString()}</p><p><strong>Seller:</strong> ${sellerName}</p>`,
    },
    metadata: { type: 'bid_won_buyer', productId },
  });
}

export async function sendBidWonSellerEmail(
  payload: Payload,
  params: { sellerEmail: string; sellerName: string; buyerName: string; buyerEmail: string; productTitle: string; productId: number; bidAmount: number; currency: string }
): Promise<{ success: boolean; emailId?: string; error?: string }> {
  const { sellerEmail, sellerName, buyerName, buyerEmail, productTitle, productId, bidAmount, currency } = params;
  const currencySymbol = CURRENCY_SYMBOLS[currency] || currency;
  return sendTemplateEmail(payload, {
    to: sellerEmail, templateType: 'bid_won_seller',
    variables: { sellerName, sellerEmail, buyerName, buyerEmail, productTitle, productId, bidAmount, currencySymbol },
    fallback: {
      subject: `Your item "${productTitle}" has been sold!`,
      html: `<h2>Congratulations ${sellerName}!</h2><p>Your item <strong>${productTitle}</strong> has been sold.</p><p><strong>Winning Bid:</strong> ${currencySymbol}${bidAmount.toLocaleString()}</p><p><strong>Buyer:</strong> ${buyerName} (${buyerEmail})</p>`,
    },
    metadata: { type: 'bid_won_seller', productId },
  });
}

export async function sendAuctionRestartedTemplateEmail(
  payload: Payload,
  params: { to: string; buyerName: string; productTitle: string; productId: number; auctionEndDate: string }
): Promise<{ success: boolean; emailId?: string; error?: string }> {
  const { to, buyerName, productTitle, productId, auctionEndDate } = params;
  return sendTemplateEmail(payload, {
    to, templateType: 'auction_restarted',
    variables: { buyerName, productTitle, productId, auctionEndDate: new Date(auctionEndDate).toLocaleString() },
    fallback: {
      subject: `Bidding has reopened for "${productTitle}"!`,
      html: `<h2>Auction Reopened!</h2><p>Hi ${buyerName},</p><p>Good news! Bidding has been reopened for <strong>${productTitle}</strong>.</p><p><strong>New auction end date:</strong> ${new Date(auctionEndDate).toLocaleString()}</p>`,
    },
    metadata: { type: 'auction_restarted', productId },
  });
}
